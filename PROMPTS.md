...
All prompts used by the application's AI features. Prompts are editable via the **Prompts** menu in the app. Placeholders like `{{variable}}` are replaced at runtime with actual values.
...
Answers questions about a news summary in an interactive chat.
...
ADEPT deliberation panel persona: Evidence Skeptic. Follows Adam Grant's "Think Again" principles to challenge beliefs as hypotheses.
...
ADEPT deliberation panel persona: Institutionalist. Represents the perspective of established institutions and consensus.
...
ADEPT deliberation panel persona: Moralist. Examines claims through ethical and values-based lenses.
...
Detects logical fallacies and cognitive vulnerabilities in text using David Robert Grimes' taxonomy and Dan Ariely's "Funnel of Misbelief."
...
Evaluates scientific study methodology reported in news headlines.
...
Tracks how misinformation narratives spread across social media platforms.
...
Analyzes health/wellness misinformation gateway pathways into conspiracy funnels.
...
Compares how different news outlets cover the same story across the political spectrum.
...
Detects Sorting, Othering, and Siloing patterns in information diet using Monica Guzman's "Fearless Curiosity" framework.
...
Generates bridge-building questions between two opposing viewpoints.
...
Detects the single most prominent manipulation technique in an article.
...
### Bias Radar — Framing Analysis

Analyzes article framing, language, omissions, and manipulation techniques. Used directly in code (not editable via Prompts UI).
...
Analyzes how story coverage evolves over time from the same news outlet.
...
Generates the strongest, most charitable counter-argument to a user's position on an article.
...
Identifies under-reported stories from a week's headlines that received notably little coverage relative to their significance.
...
Classifies job postings by relevance for a Senior Frontend Developer role and assesses remote-friendliness.
